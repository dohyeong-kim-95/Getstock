...
- **Adjusted prices**: The bulk per-date API does not support an `adjusted` parameter, so adjusted prices require per-ticker calls via `pykrx.stock.get_market_ohlcv_by_date(fromdate, todate, ticker, adjusted=True)`. **v1 implementation**: Daily runs set `adj_*` = raw values (no per-ticker HTTP calls). Backfill runs fetch per-ticker adjusted prices with 0.5s throttling. This means daily-mode adjusted values are only approximate until the next backfill refresh. This tradeoff is documented in the code (`sources/krx.py:fill_adjusted_from_raw`).
- **Dividends/Splits**: Deferred in v1. `pykrx` has limited corporate action APIs and the data is sparse. Stub functions exist but return empty DataFrames and are not called by the pipeline. The `data/dividends/` and `data/splits/` directories will not contain files in v1.
...
|---|---|---|
| `source_id` | `VARCHAR` | Source-native identifier used as primary key. KRX: 6-digit stock code (e.g., `"005930"`, stable). US: Tiingo ticker symbol (Tiingo maps old→new internally; reasonably stable but not immutable — `permaTicker` from Fundamentals API is a future upgrade). |
